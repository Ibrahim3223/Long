...
if not CV2_AVAILABLE:
    logger.warning("OpenCV (cv2) not installed - face detection will be limited")
...
class FaceDetector:
    ...
    Optimized for long-form videos with higher sample rate for efficiency.
    """
    ...
    def detect_faces_in_frame(self, frame) -> List[FaceData]:
        """
    ...
    def _analyze_face_emotion(
        ...
        For full emotion analysis, integrate with EmotionAnalyzer.
        ...
        Returns:
            (emotion, confidence)
        """
        # TODO: Integrate with EmotionAnalyzer for better accuracy
    ...
    def score_frames(
        self,
        video_path: str,
        sample_rate: int = 60,  # Higher for long-form (every 2s at 30fps)
        max_frames: int = 150   # More frames for long-form videos
    ) -> List[FrameScore]:
        """
        ...
        Args:
            ...
            sample_rate: Sample every N frames (higher for long-form)
        ...
        """
        if not CV2_AVAILABLE:
            logger.warning("OpenCV not available - returning empty frame scores")
            return []
    ...
    def get_best_frames(
        ...
        video_path: str,
        num_frames: int = 3,
        diversity_threshold: float = 0.2  # Lower for long-form (more diverse timestamps)
    ) -> List[FrameScore]:
        """
...
def _test_face_detector():
    ...
    print("Full test requires video file")
