...
- Pattern interrupts (every 15-20 seconds for long-form)
...
- Pattern interrupts (15-20s): +60% engagement
...
class CuriosityType(Enum):
    ...
    TEASER = "teaser"                     # "Coming up next..."
    FORESHADOWING = "foreshadowing"       # "Remember this for later..."
...
class CuriosityGap:
    ...
    duration: float = 2.5          # How long to display (longer for long-form)
...
class CuriosityGenerator:
    ...
    - Curiosity gaps every 15-20 seconds (vs 8-10s for shorts)
    - Varied interrupt types (no repetition)
    - Intensity progression (build up to climax)
    - Multiple climax points for longer videos
    """
    ...
    CURIOSITY_TEMPLATES = {
        ...
        CuriosityType.CONTRADICTION: [
            ...
            "Common wisdom says {assumption}, however...",
        ],
        ...
        CuriosityType.COUNTDOWN: [
            ...
            "Let's start with number {number}...",
        ],
        CuriosityType.TEASER: [
            "Coming up...",
            "Stay tuned for...",
            "Later in this video...",
    ...
    # Long-form specific intervals (seconds)
    LONG_FORM_GAP_INTERVAL = 18.0  # Every 18 seconds for long-form
    LONG_FORM_INTERRUPT_INTERVAL = 15.0  # Every 15 seconds
    ...
    def generate_curiosity_plan(
        ...
        # Calculate optimal gap placement (every 15-20 seconds for long-form)
        gap_interval = self.LONG_FORM_GAP_INTERVAL
        ...
        for i in range(num_gaps):
            ...
            # Don't place gaps too close to end (last 5 seconds)
            if timestamp > video_duration - 5.0:
                break
            ...
            gap = CuriosityGap(
                ...
                text=text,
                intensity=intensity,
                duration=2.5  # Longer display for long-form
            )
            gaps.append(gap)
    ...
    def generate_pattern_interrupts(
        ...
        # Pattern interrupt every 15 seconds for long-form (offset from curiosity gaps)
        interrupt_interval = self.LONG_FORM_INTERRUPT_INTERVAL
        ...
        interrupt_types = [
            ...
            "color_shift",       # Color grading change
    ...
    def _select_gap_type_for_position(
        ...
        # Hook phase (0-25%) -> Questions, mysteries, teasers
        if progress < 0.25:
            preferred = [CuriosityType.QUESTION, CuriosityType.MYSTERY, CuriosityType.TEASER]

        # Buildup (25-50%) -> Wait for it, contradictions, foreshadowing
        elif progress < 0.50:
            preferred = [CuriosityType.WAIT_FOR_IT, CuriosityType.CONTRADICTION, CuriosityType.FORESHADOWING]

        # Development (50-75%) -> Truth reveals, countdown
        elif progress < 0.75:
            preferred = [CuriosityType.TRUTH_REVEAL, CuriosityType.COUNTDOWN, CuriosityType.WAIT_FOR_IT]

        # Climax (75-100%) -> Surprises, cliffhangers
        else:
    ...
    def _get_ai_curiosity_gaps(
        ...
        try:
            ...
            prompt = f"""Analyze this long-form YouTube video script and generate {num_gaps} curiosity gaps for maximum retention:

Script: {script_text[:1500]}
Topic: {topic}

Place curiosity gaps every 15-20 seconds to keep viewers engaged. Each gap should:
...
{{
    "gaps": [
        {{
            "timestamp": 18.0,
            "type": "wait_for_it|truth_reveal|surprise|question|cliffhanger|contradiction|mystery|teaser|foreshadowing",
            ...
            type_map = {
                ...
                "teaser": CuriosityType.TEASER,
                "foreshadowing": CuriosityType.FORESHADOWING,
            }
            ...
            logger.warning(f"AI curiosity generation failed: {e}")
            return None
...
def _test_curiosity_generator():
    ...
    print("=" * 60)
    print("CURIOSITY GENERATOR TEST (Long-form)")
    print("=" * 60)

    generator = CuriosityGenerator()

    # Test curiosity gap plan for long-form video (3 minutes)
    print("\n[1] Testing curiosity gap generation (180s video):")
    ...
    for i, interrupt in enumerate(interrupts[:8], 1):  # First 8
